...
class CampaignOutbox(Base):
    ...
    created_at = Column(DateTime, index=True, server_default=text('current_timestamp()'))
    updated_at = Column(DateTime, server_default=text('current_timestamp()'))


t_fuelrod_migration = Table(
    'fuelrod_migration', metadata,
...
class UntrackedCallback(Base):
    ...
    id = Column(BIGINT(20), primary_key=True)
    message_id = Column(String(250, 'utf8mb4_unicode_ci'), nullable=False, comment='message identifier')
...
class Users(Base):
    ...
    user_role = Column(String(8, 'utf8mb4_unicode_ci'), nullable=False, server_default=text("'USER'"))
    flag_reason = Column(String(50, 'utf8mb4_unicode_ci'), nullable=False, server_default=text("'ACTIVE'"))
    ...
    sms_stats = relationship('SmsStats', back_populates='user')
...
class SmsCampaign(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_uuid'], ['users.uuid'], onupdate='CASCADE', name='user-campaign-uid-fk'),
    )
...
class SmsStats(Base):
    ...
    message_cost = Column(DECIMAL(10, 2), nullable=False)
    message_count = Column(BIGINT(20), nullable=False)
    message_start_date = Column(Date, nullable=False, index=True)
    message_end_date = Column(Date, nullable=False)
    user_uuid = Column(String(64, 'utf8mb4_unicode_ci'))

    user = relationship('Users', back_populates='sms_stats')


class UserApiConfig(Base):
    __tablename__ = 'user_api_config'
    __table_args__ = (
        ForeignKeyConstraint(['user_uuid'], ['users.uuid'], onupdate='CASCADE', name='user_uid_fk'),
    ...
    service_type = Column(String(10, 'utf8mb4_unicode_ci'), server_default=text("'BULK_SMS'"))
...
class UserSmsCredit(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_uuid'], ['users.uuid'], onupdate='CASCADE', name='user-credit-uid-fk'),
    ...
    remarks = Column(String(150, 'utf8mb4_unicode_ci'), nullable=False, comment='Comments regarding the top up')
    created_at = Column(DateTime, server_default=text('current_timestamp()'))
    updated_at = Column(DateTime, server_default=text('current_timestamp()'))
...
class UserTokens(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['user_name'], ['users.username'], ondelete='CASCADE', name='jwt_token_username'),
    )
...
class MessageQueue(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['campaign_id'], ['sms_campaign.id'], ondelete='CASCADE', onupdate='CASCADE',
                             name='fk-campaign-id'),
    )
